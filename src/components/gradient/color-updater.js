...
function addUpdateColorsToGradient(gradientInstance) {
  ...
  gradientInstance.updateColors = function(colors) {
    ...
    try {
      ...
      if (this.uniforms && this.uniforms.u_waveLayers && this.uniforms.u_waveLayers.value) {
        for (let i = 1; i < Math.min(normalizedColors.length, this.uniforms.u_waveLayers.value.length + 1); i++) {
          if (this.uniforms.u_waveLayers.value[i - 1] && 
              this.uniforms.u_waveLayers.value[i - 1].value && 
              this.uniforms.u_waveLayers.value[i - 1].value.color) {
            this.uniforms.u_waveLayers.value[i - 1].value.color.value = normalizedColors[i];
          }
        }
      }
